document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderProducts(list) {
    ...
    list.forEach(prod => {
      ...
      card.innerHTML = `
        ...
        <button class="add-to-cart" data-name="${prod.name}">Adicionar</button>
      `;
  ...
  function updateCart() {
    ...
    cart.forEach(item => {
      ...
      row.innerHTML = `
        ...
        <button class="remove" data-name="${item.name}">x</button>
      `;
  ...
  document.addEventListener("click", e => {
    ...
    if (e.target.classList.contains("add-to-cart")) {
      ...
      const qty = parseInt(e.target.parentElement.querySelector("input").value);
  ...
  checkoutBtn.addEventListener("click", () => {
    ...
    let msg = cart.map(i => `${i.qty}x ${i.name}`).join("\n");
    msg += `\n\nTotal: R$ ${cartTotalEl.textContent}`;
    let url = `https://wa.me/5500000000000?text=${encodeURIComponent(msg)}`;
